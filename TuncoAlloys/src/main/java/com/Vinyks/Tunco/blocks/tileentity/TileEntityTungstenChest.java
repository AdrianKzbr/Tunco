package com.Vinyks.Tunco.blocks.tileentity;
...
public class TileEntityTungstenChest extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.tungsten_chest";
	}
	...
	{
		return new ContainerTungstenChest(playerInventory, this, playerIn);
	}
    ...
    public void update()
    {
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerTungstenChest)
                {
                    if (((ContainerTungstenChest)entityplayer.openContainer).getChestInventory() == this)
                    {
